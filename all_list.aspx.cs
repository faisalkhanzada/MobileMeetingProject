...
public partial class all_list : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
      ...
      //  EventId = "2d5d649d-902f-4f02-ada4-b0a6c44d1293";
        ...
        if (!IsPostBack)
        {
            List<TempAttendees> AllAttendees = this.Session["AllAttendees" + Request.QueryString["EvtId"] + HttpContext.Current.User.Identity.Name] as List<TempAttendees>;

            
            //EngClient.GetMeetingSlots(


            if (AllAttendees == null)
            {
                List<MobileMeetingProject.EsEngine.Attendee> AttendeesAll = (from a in GetAttendees(EventId) orderby a.Name select a).ToList();
                ...
                AllAttendees = (from a in AttendeesAll join AA in Attendees on a.Id equals AA.Id orderby AA.Name ascending select new TempAttendees(a.Id, a.Name, a.CompanyName, a.JobTitle, a.CompanyWebsite, a.AllocatedTable, a.Role, a.Day1, a.Day2, a.Day3, a.CompanyId)).ToList();
                //AllAttendees.AddRange((from a in AttendeesAll orderby a.Name select new TempAttendees(a.Id, a.Name, a.CompanyName, a.JobTitle, a.CompanyWebsite, a.AllocatedTable, a.Role, a.Day1, a.Day2, a.Day3, a.CompanyId)).ToList<TempAttendees>());
            ...
            else
            {
                ...
                Attendees = (from a in Attendees orderby a.Name select a).ToList();
                AllAttendees = (from a in AttendeesAll join AA in Attendees on a.Id equals AA.Id orderby AA.Name ascending select new TempAttendees(a.Id, a.Name, a.CompanyName, a.JobTitle, a.CompanyWebsite, a.AllocatedTable, a.Role, a.Day1, a.Day2, a.Day3, a.CompanyId)).ToList();
                //AllAttendees.AddRange((from a in AttendeesAll orderby a.Name select new TempAttendees(a.Id, a.Name, a.CompanyName, a.JobTitle, a.CompanyWebsite, a.AllocatedTable, a.Role, a.Day1, a.Day2, a.Day3)).ToList<TempAttendees>());
                this.Session["AllAttendees" + Request.QueryString["EvtId"] + HttpContext.Current.User.Identity.Name] = AllAttendees;
            }
        ...
        int counter = 0;
            ...
            if(GridView1.Rows.Count > 0)
            {
                foreach (GridViewRow row in GridView1.Rows)
                {
                    ...
                    attID = row.Cells[9].Text.ToString();
                    ...
                    for (int i = 0; i < GridView1.Columns.Count; i++)
                    {
                        ...
                        lst.Add(cellText);
                        if (lst[0].StartsWith("Delegate"))
                        {
                            role = "<p><img src='images/d-icon.jpg' alt='' /></p>";
                            roleletter = lst[0];
                        }
                        else if (cellText.StartsWith("Vendor"))
                        {
                            role = "<p><img src='images/v-icon.jpg' alt='' /></p>";
                            roleletter = lst[0];
                        }
                        else if (cellText.StartsWith("Speaker"))
                        {
                            role = "<p><img src='images/s-icon.jpg' alt='' /></p>";
                            roleletter = lst[0];
                        }
                        else if (cellText.Contains("Event Partner"))
                        {
                            role = "<p><img src='images/p-icon.jpg' alt='' /></p>";
                            roleletter = lst[0];
                        }
                    }

                    String html = "<a id=" + counter + " class='venrow' data-attcountday1=" + countday1 + " data-attcountday2=" + countday2 + " data-meetingscountday1=" + hasmeetingday1 + " data-meetingscountday2=" + hasmeetingday2 + " href='Meeeting_details.aspx?attid=" + attID + "' style='color:black'><div class='listtable'><div class='ltbox6'>" + role + "</p></div> <div class='ltbox1'> <p>" + lst[1] + "</p> </div> <div class='ltbox2'> <p>" + lst[3] + "</p> </div> <div class='ltbox3'> <p><strong>" + lst[2] + "</strong></p> </div> <div class='ltbox4'> <div class='ltbox4bottom'> <div class='ltbox4left'> <p>" + countday1 + "</p> </div> <div class='ltbox4right'> <p>" + countday2 + "</p> </div> </div> </div> <div class='ltbox5'> <div class='ltbox4bottom'> <div class='ltbox4left'> <p>" + hasmeetingday1 + "</p> </div> <div class='ltbox4right'> <p>" + hasmeetingday2 + "</p> </div> </div> </div> </div></a>";
                    list += html;
                    roleletter = "";
                    lst.Clear();
                }
